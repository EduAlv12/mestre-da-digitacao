...
test('RPG usa a mesma ação compartilhada',()=>{assert.match(typing,/id='rpg'/);assert.match(typing,/continueBattle/);assert.doesNotMatch(rpg,/rpg-continue-btn/)});
...
test('prévia não fecha o modal nem salva a seleção',()=>{assert.match(ui,/modal-sound&&e\.target\.closest\('\.sound-preview-btn'\)\)return/);assert.match(ui,/audioEngine\.playPreview\(value\)/)});
