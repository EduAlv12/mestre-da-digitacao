...
(() => {
  const init = () => {
    ...
    const style=document.createElement('style');style.id='keyboard-focus-style';style.textContent=`
      body,.app-card,.app-card button,.app-card label,.app-card span,.app-card p,.app-card h1,.app-card h2,.app-card h3,.app-card div:not(#hidden-input){
      ...
      .mode-status-info-row{display:flex;align-items:center;justify-content:center;gap:6px;min-width:0;}
      ...
      #continue-mode-btn{grid-column:1 / -1;width:100%;min-height:46px;justify-self:stretch;}
    `;document.head.appendChild(style);
    document.addEventListener('contextmenu',event=>{if(!event.target.closest('input,textarea,a'))event.preventDefault();},{passive:false});
    ...
    input.addEventListener('focus',()=>wrapper.classList.add('typing-focused'));input.addEventListener('blur',()=>wrapper.classList.remove('typing-focused'));wrapper.addEventListener('pointerdown',focusTypingInput);wrapper.addEventListener('click',focusTypingInput);
  };
