...
export default {
  ...
  startTimer(){this.stopTimer();this.timerId=setInterval(()=>{this.timeLeft=Math.max(0,this.timeLeft-1);this.updateUI();if(this.timeLeft<=0){this.stopTimer();const accuracy=this.totalTypedChars?Math.round(((this.totalTypedChars-this.totalErrors)/this.totalTypedChars)*100):100,wpm=Math.round(this.wordsTyped/(this.timeLimit/60));document.dispatchEvent(new CustomEvent('modeEndTest',{detail:{accuracy,wpm,modeId:this.id}}));}},1000);},
