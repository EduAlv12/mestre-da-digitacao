...
export function disableTimerMode() {
  if (state.isTimerMode) {
    ...
    if (btn) {
      btn.textContent = '⏱️ Contra-Relógio';
...
function openModeHelp() {
  ...
  const id = getModeId();
  const help = MODE_HELP[id] || MODE_HELP.default;
...
export function renderHistoryChart(modeId) {
  ...
  const history = stats.ppmHistory || [];
  const maxPPM = Math.max(10, ...history, 1);
  chart.innerHTML = '';
  if (history.length === 0) {
    chart.innerHTML = '<span style="font-size:0.6rem;color:var(--text-muted);width:100%;text-align:center;padding:8px 0;">Nenhum dado ainda. Comece a digitar!</span>';
    if (avgEl) avgEl.textContent = 'Média: 0';
    return;
  }
  const avg = Math.round(history.reduce((a, b) => a + b, 0) / history.length);
  if (avgEl) avgEl.textContent = `Média: ${avg}`;
  history.forEach(value => {
    const dot = document.createElement('div');
    dot.className = `history-dot ${value > 0 ? 'active' : ''}`;
    const h = Math.max(10, (value / maxPPM) * 80);
    dot.style.setProperty('--value', h);
    dot.style.height = `${Math.max(4, h * 0.48)}px`;
    if (value > 0) {
      const tip = document.createElement('span');
      tip.className = 'dot-tooltip';
      tip.textContent = `${value} PPM`;
      dot.appendChild(tip);
    }
...
function updateTutorialUI() {
  ...
  steps.forEach((el, i) => el.classList.toggle('active', i === tutorialStep));
  dots.forEach((el, i) => el.classList.toggle('active', i === tutorialStep));
...
function setupTutorial() {
  ...
  if (prevBtn) {
    prevBtn.addEventListener('click', (e) => {
      e.preventDefault();
      if (tutorialStep > 0) { tutorialStep--; updateTutorialUI(); }
    });
  }
...
export function setDifficulty(val) {
  state.currentDifficulty = val;
  if (difficultyTriggerText) difficultyTriggerText.textContent = DIFFICULTY_NAMES[val] || 'Fácil';
  if (modalDifficulty) {
  ...
  if (standardMedals) standardMedals.classList.toggle('hidden', isCustom);
  if (customMedals) customMedals.classList.toggle('hidden', !isCustom);
  if (medalsTitle) medalsTitle.textContent = isCustom ? '🏅 Medalhas (Modo Personalizado)' : '🏅 Medalhas (Modo Padrão)';
  if (editCustomBtn) editCustomBtn.classList.toggle('hidden', !isCustom);
  if (restartBtn) restartBtn.textContent = '↻ Recomeçar';
  if (difficultyTag) difficultyTag.textContent = `Nível: ${DIFFICULTY_NAMES[val]}`;

  updateMedalLabels();
  loadAchievements();
  initTest();
  if (isCustom && (!state.customUserText || state.customUserText.trim().length < 10)) {
...
function updateMedalLabels() {
  ...
  const limits = DIFFICULTY_THRESHOLDS[diff];
...
export function setTheme(val) {
  ...
  if (themeTriggerText) themeTriggerText.textContent = THEME_NAMES[val] || 'Azul Marinho';
  if (modalTheme) {
...
export function setSoundProfile(val) {
  ...
  if (soundTriggerText) soundTriggerText.textContent = SOUND_NAMES[val] || 'Thock Mecânico';
  if (modalSound) {
...
export function loadSavedSettings() {
  ...
  const savedVolume = localStorage.getItem('selectedSoundVolume') || '0.9';
...
export function setupModalTriggers() {
  document.addEventListener('click', (e) => {
    const target = e.target.closest('.select-trigger, .btn-achievements-trigger');
    if (!target) return;
    if (target.id === 'mode-trigger') {
      renderModeList();
      openModal(modalModes);
    } else if (target.id === 'difficulty-trigger') {
      openModal(modalDifficulty);
    } else if (target.id === 'theme-trigger') {
      openModal(modalTheme);
    } else if (target.id === 'sound-trigger') {
      openModal(modalSound);
    } else if (target.id === 'achievements-trigger') {
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      [modalDifficulty, modalTheme, modalSound, modalModes, modalCustomText, modalAchievements, modalPpmInfo, modalWelcome, modalRestartConfirm].forEach(m => closeModal(m));
    }
  });

  document.addEventListener('click', (e) => {
    const option = e.target.closest('.modal-option-btn');
    if (!option) return;
    const parentModal = option.closest('.modal-overlay');
    if (!parentModal) return;
    const value = option.getAttribute('data-value');
    if (!value) return;

    if (parentModal.id === 'modal-difficulty') {
      setDifficulty(value);
      closeModal(parentModal);
    } else if (parentModal.id === 'modal-theme') {
      setTheme(value);
      closeModal(parentModal);
    } else if (parentModal.id === 'modal-sound') {
      setSoundProfile(value);
      closeModal(parentModal);
    } else if (parentModal.id === 'modal-modes') {
      setMode(value);
      closeModal(parentModal);
      if (modeTriggerText) {
        modeTriggerText.textContent = MODE_NAMES[value] || value;
      }
      const stats = getModeStats(value);
      const bestEl = document.getElementById('best-ppm-val');
      if (bestEl) bestEl.textContent = stats.bestPPM || 0;
      renderHistoryChart(value);
      updateGlobalLevelUI();
    }
  });
  ...
  document.addEventListener('click', (e) => {
    ...
    e.stopPropagation();
    const value = previewBtn.getAttribute('data-value');
    if (!value) return;
    const currentProfile = audioEngine.profile;
    audioEngine.setProfile(value);
    audioEngine.playPreview();
    setTimeout(() => {
      audioEngine.setProfile(currentProfile);
    }, 500);
  });

  document.addEventListener('click', (e) => {
    if (e.target.closest('#ppm-info-btn')) {
      openModal(modalPpmInfo);
    }
  });
  ...
  if (saveCustomTextBtn) {
    saveCustomTextBtn.addEventListener('click', () => {
      ...
      state.customUserText = val;
      localStorage.setItem('customUserText', val);
  ...
  if (editCustomBtn) {
    editCustomBtn.addEventListener('click', () => {
      if (customTextInput) customTextInput.value = state.customUserText;
  ...
  if (confirmRestartBtn) {
    confirmRestartBtn.addEventListener('click', (e) => {
      e.preventDefault();
      closeModal(modalRestartConfirm);
      initTest();
    });
  }
  ...
  if (feedbackBtn) {
    feedbackBtn.addEventListener('click', (e) => {
      e.preventDefault();
      window.open('https://forms.gle/KsACEZfVzteRzm8s9', '_blank');
    });
  }
}
...
export function toggleTimerMode() {
  ...
  if (mode && mode.hasTimer) {
    showToast('⚠️ Este modo já possui temporizador próprio!');
    return;
  }
  ...
  const btn = document.getElementById('timer-mode-btn');
  if (btn) {
    btn.textContent = state.isTimerMode ? '⏱️ Modo Normal' : '⏱️ Contra-Relógio';
    btn.classList.toggle('active', state.isTimerMode);
  }
  if (countdownTag) {
    if (state.isTimerMode) {
      countdownTag.textContent = `⏱️ ${state.timerModeLimit}s`;
...
export function setupShareButton() {
  ...
  btn.addEventListener('click', () => {
    ...
    if (navigator.share) {
      navigator.share({ title: 'Mestre da Digitação', text, url }).catch(() => {});
    } else if (navigator.clipboard) {
      navigator.clipboard.writeText(`${text}\n\n${url}`).then(() => {
        showToast('✅ Link copiado! Compartilhe com seus amigos.');
        btn.textContent = '✅ Copiado!';
        btn.classList.add('success');
        setTimeout(() => { btn.textContent = '📤 Compartilhar'; btn.classList.remove('success'); }, 2000);
