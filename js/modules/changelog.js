...
function buildChangelog() {
  ...
  modal.innerHTML = `<div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="changelog-title">
    <div class="modal-header"><h3 class="modal-title" id="changelog-title">🆕 O que mudou?</h3><button type="button" class="modal-close" aria-label="Fechar">&times;</button></div>
    <div class="modal-body"><p>Veja as melhorias e correções mais recentes do Mestre da Digitação.</p><div class="changelog-list"></div><div class="changelog-footer">As novidades aparecem aqui para você saber o que mudou sem precisar conhecer o código.</div></div>
  </div>`;
  document.body.appendChild(modal);

  const list = modal.querySelector('.changelog-list');
  list.innerHTML = CHANGELOG.map(section => `<section><div class="changelog-version">${section.version}<span class="changelog-date">${section.date}</span></div>${section.items.map(item => `<div class="changelog-item">${item}</div>`).join('')}</section>`).join('');
