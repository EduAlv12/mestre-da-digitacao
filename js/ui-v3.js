...
(() => {
  ...
  const init=()=>{
    ...
    const helpGroup=document.getElementById('mode-help-trigger')?.closest('.control-group');
    const status=document.getElementById('mode-status-tag');
    ...
    const timerImportNotice=document.querySelector('.tutorial-step[data-step="3"] .tutorial-text');const timerDemoNotice=document.querySelector('.tutorial-step[data-step="3"] .tutorial-demo');if(timerImportNotice)timerImportNotice.innerHTML='Personalize <strong>Temas</strong> e <strong>Sons</strong> para deixar seu treino do seu jeito.';if(timerDemoNotice)timerDemoNotice.textContent='🎨 Temas • 🔊 Sons';
    ...
    open.addEventListener('click',show);head.querySelector('.ui-v3-close').addEventListener('click',close);backdrop.addEventListener('click',close);document.addEventListener('keydown',e=>{if(e.key==='Escape')close()});
